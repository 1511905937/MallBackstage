...
public class User {
	  public int getid(String name,String pwd) throws SQLException, ClassNotFoundException
	  {
		...
		public String register(String name,String pwd,String role) throws ClassNotFoundException 
		{
			...
			if(role=="1")          //锟斤拷锟斤拷锟�
				Rrole=1;
			else if(role=="2")         //锟斤拷锟斤拷锟斤拷
				Rrole=2;
			else
				return "2";            //注锟斤拷失锟杰ｏ拷没锟叫憋拷锟斤拷锟斤拷锟�
		...
		public List<Productinfo> userlike(int id) throws SQLException, ClassNotFoundException{
			List<Productinfo> list=new ArrayList<Productinfo>();
			String sql="select * from userlike,products where userlike_userid="+id+" and userlike.userlike_proid=products.pro_id";
			ResultSet rs=null;
			Sql s=new Sql();
			rs=s.select(sql);
				while(rs.next())
		...
		public String add_to_like(int userid,int pro_id) throws ClassNotFoundException 
		{
			
			String sql="insert into userlike values(null,"+userid+","+pro_id+")";
			Sql s=new Sql();
			int result=0;
			result=s.update(sql);
			if(result==1)       //锟斤拷锟斤拷晒锟�
